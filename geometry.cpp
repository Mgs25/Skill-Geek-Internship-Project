...
TwoDShape::TwoDShape(int d) : Shape(d) {
    // No additional functionality or operations to be performed
}
...
bool shadePoint(const Scene& s, const Point& p) {
    ...
    for (auto P: s.objectList) { // P is the pair(int, vector<shared_ptr<Shape>>)
        for (auto listItem: P.second) { // listItem is the vector of Shape pointers, which is the second item in the pair
